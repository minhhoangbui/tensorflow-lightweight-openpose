...
class CPM(tf.keras.layers.Layer):
    def __init__(self, out_channels):
        ...
        self.cpm = Conv(out_channels=out_channels, bn=False, name='cpm_cpm')
...
class InitialStage(tf.keras.layers.Layer):
    def __init__(self, num_channels, num_heatmaps, num_pafs):
        ...
        self.trunk = tf.keras.Sequential([
            Conv(num_channels, bn=False, name='InitialStage_conv1'),
            Conv(num_channels, bn=False, name='InitialStage_conv2'),
            Conv(num_channels, bn=False, name='InitialStage_conv3')
        ])
        self.heatmaps = tf.keras.Sequential([
            Conv(512, kernel_size=1, bn=False, name='InitialStage_conv4'),
            Conv(num_heatmaps, kernel_size=1, bn=False, relu=False, name='InitialStage_heat')
        ])
        self.pafs = tf.keras.Sequential([
            Conv(512, kernel_size=1, bn=False, name='InitialStage_conv5'),
...
class RefinementStage(tf.keras.layers.Layer):
    def __init__(self, num_channels, num_heatmaps, num_pafs):
        ...
        self.trunk = tf.keras.Sequential([
            ...
            RefinementBlock(num_channels, name='RefinementStage_block2'),
            RefinementBlock(num_channels, name='RefinementStage_block3'),
            RefinementBlock(num_channels, name='RefinementStage_block4'),
            RefinementBlock(num_channels, name='RefinementStage_block5')
        ])
        self.heatmaps = tf.keras.Sequential([
            Conv(512, kernel_size=1, bn=False, name='RefinementStage_conv1'),
            Conv(num_heatmaps, kernel_size=1, bn=False, relu=False, name='RefinementStage_heat')
        ])
        self.pafs = tf.keras.Sequential([
            Conv(512, kernel_size=1, bn=False, name='RefinementStage_conv'),
...
class LightWeightOpenPose(tf.keras.Model):
    def __init__(self, num_channels, num_refinement_stages=1, num_joints=19, num_pafs=38):
        ...
        self.cpm = CPM(num_channels)
        self.initial_stage = InitialStage(num_channels, num_joints, num_pafs)
        self.refinement_stages = [
            RefinementStage(num_channels, num_joints, num_pafs) for _ in range(num_refinement_stages)
        ]
