...
def main():
    ...
    while (cap.isOpened() or completed_request_results
    ...
    for mode_value, mode_stats in mode_info.items():
        ...
        log.info('FPS: {:.1f}'.format(mode_stats.frames_count / (end_time - mode_stats.last_start_time)))
        log.info('Latency: {:.1f} ms'.format((mode_stats.latency_sum / mode_stats.frames_count) * 1e3))
